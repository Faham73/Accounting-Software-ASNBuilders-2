...
export async function getProjectTotals(
  ...
  // Labor by type: only available after Prisma client is regenerated (schema has type field).
  // Fallback: if client doesn't know about type, treat all labor as DAY so the app doesn't crash.
  let laborByType: LaborByType = { DAY: 0, MONTHLY: 0 };
  try {
    const [dayAgg, monthlyAgg] = await Promise.all([
      prisma.projectLabor.aggregate({
